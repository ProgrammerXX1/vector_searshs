...
# ---------- парсер: ПОСТАНОВЛЕНИЕ о признании лица гражданским истцом ----------
def parse_priznanie_poter_graj(text: str, filename: Optional[str] = None) -> Dict[str, Optional[str]]:
    """
    Парсер под вид:
    ...
    is_priznanie = bool(re.search(
        r"постановлени\w*[\s\r\n]+о\s+признани[ея]\s+лица\s+гражданским\s+истцом",
        heading, re.I | re.S
    ))

    type_document = "Постановление о признании лица гражданским истцом" if is_priznanie else "Постановление"
